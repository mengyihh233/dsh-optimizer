...
function inspectPatch(file: string): PatchState {
  ...
  if (!src.includes(PAGINATE_FROM) && !src.includes(PAGINATE_TO)) details.push('分页返回行未找到，部署版本可能已变更')
...
function revertPatch(file: string): PatchState {
  const src = readFileSync(file, 'utf8')
  if (!src.includes(PATCH_START) && !src.includes(PAGINATE_TO)) return { file, patched: false, details: ['未检测到补丁'] }
  let out = src
  // 移除补丁块
