...
function OptimizerSettings(props) {
    ...
    const runScan = React.useCallback(async () => {
        setLoading(true);
        try {
            const r = (await host.call('optimizer/scan'));
            setScan(r);
            if (!r.ok)
                pushLog('扫描失败: ' + (r.message ?? '未知错误'));
            else
                pushLog(`扫描完成：${r.stats?.total ?? 0} 会话 / ${r.stats?.totalMB ?? 0}MB，发现 ${r.issues?.length ?? 0} 项`);
        }
        catch (e) {
    ...
    const doFix = React.useCallback(async (id, fix, label) => {
        setApplying(id);
        try {
            const r = (await host.call('optimizer/apply', { fix }));
    ...
    const fixAll = React.useCallback(async () => {
        ...
        try {
            for (const issue of scan.issues) {
                if (!issue.fix || issue.fix === 'none')
                    continue;
                const r = (await host.call('optimizer/apply', { fix: issue.fix }));
    ...
    return React.createElement('div', { className: 'dsh-opt-root' }, React.createElement('div', { className: 'dsh-opt-card' }, React.createElement('h3', null, '一键优化'), React.createElement('div', { className: 'dsh-opt-stats' }, React.createElement('div', { className: 'dsh-opt-stat' }, React.createElement('b', null, String(patch?.patched ?? '—')), React.createElement('span', null, '分页补丁')), React.createElement('div', { className: 'dsh-opt-stat' }, React.createElement('b', null, String(stats?.total ?? '—')), React.createElement('span', null, '会话总数')), React.createElement('div', { className: 'dsh-opt-stat' }, React.createElement('b', null, String(stats?.totalMB ?? '—')), React.createElement('span', null, '占用 (MB)')), React.createElement('div', { className: 'dsh-opt-stat' }, React.createElement('b', null, String(stats?.empty ?? '—')), React.createElement('span', null, '空会话'))), React.createElement('div', { style: { display: 'flex', gap: '8px', marginTop: '4px' } }, React.createElement('button', {
        className: 'dsh-opt-btn dsh-opt-btn-primary',
        disabled: loading,
        onClick: runScan,
    }, loading ? '扫描中…' : '一键扫描'), React.createElement('button', {
        className: 'dsh-opt-btn',
        disabled: applying !== null || fixable.length === 0,
        onClick: fixAll,
    }, applying === 'all' ? '优化中…' : `一键优化全部 (${fixable.length})`))), React.createElement('div', { className: 'dsh-opt-card' }, React.createElement('h3', null, '发现的可优化问题'), ...(scan === null
        ? [React.createElement('div', { className: 'dsh-opt-empty' }, '正在扫描…')]
        : scan.issues && scan.issues.length > 0
            ? scan.issues.map(issue => React.createElement('div', { className: 'dsh-opt-row', key: issue.id }, React.createElement('div', { className: 'dsh-opt-row-main' }, React.createElement('div', { className: 'dsh-opt-row-title' }, React.createElement('span', { className: `dsh-opt-badge dsh-opt-badge-${issue.severity}` }, severityLabel(issue.severity)), React.createElement('span', null, issue.title)), React.createElement('div', { className: 'dsh-opt-row-detail' }, issue.detail)), issue.fix && issue.fix !== 'none'
                ...
                : null))
            : [React.createElement('div', { className: 'dsh-opt-empty' }, '暂无数据')])), React.createElement('div', { className: 'dsh-opt-card' }, React.createElement('h3', null, '操作日志'), React.createElement('div', { className: 'dsh-opt-log' }, log.length === 0 ? '暂无操作。点击「一键扫描」开始。' : log.join('\n'))));
}
export function apply(ctx) {
    ...
    if (slots === undefined)
        return;
    slots.inject('settings.section', () => slots.register({ name: 'settings.section', id: 'optimizer', order: 50, label: '优化' }, (props) => React.createElement(OptimizerSettings, { close: props.close })));
}
