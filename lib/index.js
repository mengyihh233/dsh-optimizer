...
function revertPatch(file) {
    const src = readFileSync(file, 'utf8');
    if (!src.includes(PATCH_START) && !src.includes('HISTORY_SKIPPED_TYPES'))
...
async function scanIssues() {
    ...
    if (chunkHeavy.length > 0) {
        // 只统计「可归档」的：>2MB 且 24h 未动（避免动活跃会话）
        const staled = chunkHeavy.filter(s => s.mtimeMs < Date.now() - 24 * 3600000);
        issues.push({
            id: 'chunk-heavy-sessions',
            severity: 'medium',
            title: `${chunkHeavy.length} 个大会话（>2MB，通常 99% 是流式 chunk）`,
            detail: `最大 ${chunkHeavy[0] ? fmtMB(chunkHeavy[0].sizeBytes) : ''}。切到这些会话最慢；补丁已缓解，${staled.length} 个 24h 未动可归档（移入 sessions-archive，可恢复）。`,
            fix: staled.length > 0 ? 'archive_chunk_heavy' : 'none',
...
async function applyFix(fix) {
    switch (fix) {
        case 'apply_patch': {
            const file = findHostApiproxyIndex();
            if (file === null)
                return { ok: false, message: '未找到 dsh-host-apiproxy 部署文件' };
        ...
        case 'archive_chunk_heavy': {
            const sessions = await scanSessions(SESSIONS_ROOT);
            const targets = sessions.filter(s => !s.empty && s.sizeBytes > 2 * 1048576 && s.mtimeMs < Date.now() - 24 * 3600000);
            let moved = 0;
            for (const s of targets) {
                try {
                    const destWs = join(ARCHIVE_ROOT, s.workspace);
            ...
            return { ok: true, message: `已归档 ${moved}/${targets.length} 个 24h 未动的大会话（可恢复）` };
        }
        default:
...
export function apply(ctx) {
    ...
    const rpcHandlers = {
        ...
        'optimizer/apply': async (args) => {
            ...
            if (!fix)
                return { ok: false, message: '缺少 fix 参数' };
            try {
    ...
    if (webServer !== undefined && typeof webServer.register === 'function') {
        webServer.register({
            ...
            handler: async (req, res) => {
                ...
                if (request !== undefined && typeof request[Symbol.asyncIterator] === 'function') {
