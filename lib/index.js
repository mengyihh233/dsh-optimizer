...
function inspectPatch(file) {
    ...
    if (hasMarker && !hasCutof)
        details.push('部分补丁：paginate 已改，historyCutOf 未改');
...
function applyPatch(file) {
    ...
    if (!now.includes(CUTOF_TO)) {
        if (!now.includes(CUTOF_FROM))
            return { file, patched: false, details: [...details, 'historyCutOf 目标行未找到，无法应用'] };
...
function revertPatch(file) {
    ...
    const hasCutof = src.includes(CUTOF_TO);
    if (!hasMarker && !src.includes(PAGINATE_TO) && !hasCutof)
